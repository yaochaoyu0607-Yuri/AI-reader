...
def do_job(mp=None,task:MessageTask=None):
        ...
        print("执行任务")
        # message_type=2: daily digest for yesterday (read from DB, no crawling).
        if task is not None and getattr(task, "message_type", None) == 2:
            try:
                from datetime import timedelta
                from core.db import DB

                # Container TZ is Asia/Shanghai, datetime.now() follows TZ.
                ...
                arts = (
                    ...
                    .filter(Article.mp_id == mp.id)
                    .filter(Article.publish_time >= start_ts)
                    .filter(Article.publish_time < end_ts)
                    .order_by(Article.publish_time.asc())
                    .all()
                )

                from jobs.webhook import MessageWebHook
                tms = MessageWebHook(task=task, feed=mp, articles=arts)
                web_hook(tms)
                print_success(f"任务({task.id})[{mp.mp_name}]执行成功,{len(arts)}成功条数")
            except Exception as e:
                print_error(e)
            return
        all_count=0
